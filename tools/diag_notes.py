...
if sys.maxsize > 2**32:
    print("\n  [i] Python 64-битный. Это верно для Notes 9.0.1 FP10 и новее — они 64-битные.")
    print("      Разрядность стоит перепроверять только на клиентах старше.")
...
if session is None:
    ...
    if db is not None:
        ...
        if present == 0:
            print("  ВЫВОД: ни один метод не доступен через COM.")
            print("         Если версия Notes выше 8.0 (см. раздел 5), причина НЕ в возрасте")
            print("         библиотеки — эти методы просто не вынесены в COM-интерфейс.")
            print("         Запустите probe_unread.py, он отличит это от проблем связывания.")
        elif present < len(checks):
            print("  ВЫВОД: интерфейс неполный — часть методов есть, часть нет.")
        else:
